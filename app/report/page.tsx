...
export default function ReportPage() {
  ...
  const [activeSection, setActiveSection] =
    useState<ReportSection>("summary");
  ...
  const generateReport = async () => {
    ...
    try {
      const res = await fetch("/api/report", {
        ...
        body: JSON.stringify({
          text: fileExtract.text,
          fileName: fileExtract.fileName,
          model,
        }),
      });
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      report.examTopics.forEach((t) => {
        addText(
          `• [${t.importance.toUpperCase()}] ${t.topic}`,
          10,
          true
        );
        addText(`  ${t.detail}`, 9);
      ...
      doc.save(
        `DocuMind-Report-${report.documentName.replace(/[^a-z0-9]/gi, "_")}.pdf`
      );
    } catch (err) {
  ...
  return (
    <PageLayout
      breadcrumb={[{ label: "AI Tools", highlight: true }, { label: "Report" }]}
      title="Generate Report"
      description="Full document analysis — summary, Q&A, exam prep, and insights in one PDF."
      actions={
        <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            <div className="p-3 rounded-xl bg-white/[0.03] border border-white/8 text-xs text-gray-400 space-y-1">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white">
                  {fileExtract.charCount.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between">
                <span>Pages</span>
                <span className="text-white">{fileExtract.pageCount}</span>
              </div>
            </div>
          )}

          {error && (
            <ErrorBanner message={error} onRetry={generateReport} />
          )}
          ...
          {report && (
            ...
            >
              <RefreshCw size={13} aria-hidden="true" /> New Report
            </button>
          )}
        </div>
        ...
        <div className="lg:col-span-3">
          {loading ? (
            <div
              className="bg-[#111320] border border-white/8 rounded-2xl p-8 space-y-4"
              ...
              >
                {SECTION_CONFIG.map((sec) => (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-1.5 px-4 py-3 text-xs font-medium whitespace-nowrap transition-colors border-b-2",
                    ...
                    {activeSection === "keyPoints" && (
                      <ul className="space-y-2.5">
                        {report.keyPoints.map((pt, i) => (
                          ...
                          >
                            <span
                              className="w-5 h-5 rounded-full bg-purple-500/20 text-purple-400 text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5"
                              aria-hidden="true"
                            >
                              {i + 1}
                            </span>
                      ...
                      <div className="space-y-3">
                        {report.examTopics.map((t, i) => (
                          ...
                          >
                            <span
                              className={cn(
                                "text-[10px] font-bold px-2 py-1 rounded-full shrink-0 mt-0.5",
                                t.importance === "high"
            ...
            <div className="bg-[#111320] border border-white/8 rounded-2xl p-12 flex flex-col items-center justify-center text-center">
              <Sparkles
                size={36}
                className="text-purple-500/30 mb-3"
                aria-hidden="true"
              />
              <p className="text-gray-500 text-sm">
                Upload a document and click
              </p>
              <p className="text-gray-500/70 text-xs mt-0.5">
                &quot;Generate Full Report&quot; to begin
              </p>
            </div>
          )}
        </div>
