...
import { FileText, Copy, Check, Loader2, Sparkles, RefreshCw } from "lucide-react";
...
export default function SummarizePage() {
  ...
  return (
    <PageLayout
      breadcrumb={[{ label: "AI Tools", highlight: true }, { label: "Summarize" }]}
      title="Summarize"
      description="Turn any document into a clear, concise summary."
        ...
        <div className="space-y-4">
          ...
          {mode === "paste" ? (
            <textarea
              ...
              rows={12}
              aria-label="Document text to summarize"
              className="w-full px-4 py-3 rounded-xl bg-[#111320] border border-white/8 text-white placeholder-gray-500 text-sm focus:outline-none focus:border-purple-500/50 resize-none scrollbar-thin"
            />
          ) : (
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2" role="radiogroup" aria-label="Summary length">
              {lengthOptions.map((opt) => (
                ...
                >
                  <p
                    className={cn(
                      "text-sm font-medium",
                      length === opt.value ? "text-purple-300" : "text-white"
                    )}
                  >
                    {opt.label}
                  </p>
        ...
        <div className="bg-[#111320] border border-white/8 rounded-2xl overflow-hidden flex flex-col min-h-[400px]">
          <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
            <div className="flex items-center gap-2">
              <FileText size={15} className="text-purple-400" aria-hidden="true" />
            ...
            {loading ? (
              ...
              <div className="space-y-1">
                {summary.split("\n").map((line, i) => {
                  if (line.startsWith("## "))
                    return (
                      <h2 key={i} className="text-lg font-bold text-white mt-1 mb-2">
                        {line.replace(/^##\s*/, "")}
                      </h2>
                    );
                  if (line.startsWith("### "))
                    return (
                      <h3
                        key={i}
                        className="text-sm font-semibold text-purple-300 mt-4 mb-1.5"
                      >
                        {line.replace(/^###\s*/, "")}
                      </h3>
                    );
                  ...
                  if (line.startsWith("- "))
                    return (
                      <li
                        key={i}
                        className="text-sm text-gray-300 leading-relaxed ml-3 list-disc"
                      >
                  ...
                  if (/^\d+\./.test(line))
                    return (
                      <li
                        key={i}
                        className="text-sm text-gray-300 leading-relaxed ml-3 list-decimal"
                      >
                        {line.replace(/^\d+\.\s/, "")}
                      </li>
                    );
